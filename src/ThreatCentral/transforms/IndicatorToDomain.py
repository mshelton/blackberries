...
@configure(
    label='Indicator To Domain',
    description='Get all Domains linked to Indicator',
    uuids=['threatcentral.v2.IndicatorToDomain'],
            ...
            try:
                ...
                if len(indicator.get('observables', list())) is not 0:
                    for observable in indicator.get('observables'):
                        if upper(observable.get('type', dict()).get('value')) == 'DOMAIN':
